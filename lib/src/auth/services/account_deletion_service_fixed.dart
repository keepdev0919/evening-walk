...
class AccountDeletionService {
  ...
  Future<AccountDeletionResult> deleteAccount() async {
    ...
    try {
      ...
      // 4단계: Storage 데이터 삭제 (재시도 포함, 실패해도 계속 진행)
      ...
      try {
        ...
        LogService.warning('AccountDeletion', '[단계 3/4] Storage 데이터 삭제 실패하지만 계속 진행: $e');
        completedSteps.add('Storage데이터삭제실패');
      }
  ...
  String _getFailedStep(List<String> completedSteps) {
    ...
    } else if (!completedSteps.contains('Storage데이터삭제') && !completedSteps.contains('Storage데이터삭제실패')) {
  ...
  Future<void> _deleteProfileImages(String uid) async {
    try {
      ...
      LogService.info('AccountDeletion',
          'profile_images/$uid에서 ${result.items.length}개 파일 발견');
      ...
      if (result.items.isNotEmpty) {
        ...
        for (int i = 0; i < result.items.length; i++) {
          ...
          try {
            LogService.info('AccountDeletion',
                '프로필 파일 삭제 중 (${i + 1}/${result.items.length}): ${fileRef.fullPath}');
            await fileRef.delete();
            LogService.info(
                'AccountDeletion', '프로필 파일 삭제 성공: ${fileRef.fullPath}');
          } catch (fileError) {
            LogService.error('AccountDeletion',
                '개별 프로필 파일 삭제 실패: ${fileRef.fullPath} - $fileError');
            throw Exception(
                'Storage 프로필 파일 삭제 실패: ${fileRef.fullPath} - $fileError');
          }
        }

        LogService.info('AccountDeletion',
            'profile_images/$uid 폴더의 모든 파일 삭제 완료 (${result.items.length}개)');
      } else {
        LogService.info(
            'AccountDeletion', 'profile_images/$uid 폴더에 삭제할 파일이 없음');
      }
    } catch (e) {
      LogService.error('AccountDeletion', 'profile_images/$uid 폴더 삭제 실패: $e');

      // 403 에러에 대한 구체적인 안내 메시지
      if (e.toString().contains('403') ||
          e.toString().contains('Permission denied')) {
        final currentUser = _auth.currentUser;
        LogService.error('AccountDeletion', '프로필 이미지 403 권한 에러 상세 정보:');
        LogService.error('AccountDeletion', '- 요청 UID: $uid');
        LogService.error(
        ...
        throw Exception(
            'Storage 프로필 이미지 권한 에러 (403): 사용자 인증 상태를 확인해주세요. UID: $uid, 현재 Auth: ${currentUser?.uid}');
      }
  ...
  Future<void> _deleteUserPhotos(String uid) async {
    try {
      ...
      LogService.info('AccountDeletion',
          'user_photos/$uid에서 ${result.items.length}개 파일 발견');
      ...
      if (result.items.isNotEmpty) {
        ...
        for (int i = 0; i < result.items.length; i++) {
          ...
          try {
            LogService.info('AccountDeletion',
                '파일 삭제 중 (${i + 1}/${result.items.length}): ${fileRef.fullPath}');
            await fileRef.delete();
            LogService.info('AccountDeletion', '파일 삭제 성공: ${fileRef.fullPath}');
          } catch (fileError) {
            LogService.error('AccountDeletion',
                '개별 파일 삭제 실패: ${fileRef.fullPath} - $fileError');
            throw Exception(
                'Storage 파일 삭제 실패: ${fileRef.fullPath} - $fileError');
          }
        }

        LogService.info('AccountDeletion',
            'user_photos/$uid 폴더의 모든 파일 삭제 완료 (${result.items.length}개)');
      } else {
      ...
      if (verificationResult.items.isNotEmpty) {
        LogService.warning('AccountDeletion',
            '삭제 후에도 user_photos/$uid에 ${verificationResult.items.length}개 파일이 남아있음');
        throw Exception(
            'Storage 폴더 삭제 검증 실패: 여전히 ${verificationResult.items.length}개 파일이 남아있음');
      } else {
      ...
      if (e.toString().contains('403') ||
          e.toString().contains('Permission denied')) {
        final currentUser = _auth.currentUser;
        LogService.error('AccountDeletion', '403 권한 에러 상세 정보:');
        LogService.error('AccountDeletion', '- 요청 UID: $uid');
        LogService.error(
            'AccountDeletion', '- 현재 Auth UID: ${currentUser?.uid}');
        LogService.error('AccountDeletion',
            '- Auth 상태: ${currentUser != null ? "로그인됨" : "로그아웃됨"}');
        LogService.error('AccountDeletion', '- 이메일: ${currentUser?.email}');

        throw Exception(
            'Storage 권한 에러 (403): 사용자 인증 상태를 확인해주세요. UID: $uid, 현재 Auth: ${currentUser?.uid}');
      }
